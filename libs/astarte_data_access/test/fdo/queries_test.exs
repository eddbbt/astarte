...
defmodule Astarte.DataAccess.FDO.QueriesTest do
  ...
  describe "fetch_device_id_and_ownership_voucher/1" do
    setup :setup_voucher
    ...
    test "returns the voucher and the device_id", context do
      ...
      assert Queries.fetch_device_id_and_ownership_voucher(guid) ==
               {:ok, {device_id, voucher_data, realm_name}}
    end

    test "returns :not_found for invalid guid", context do
      %{realm_name: realm_name} = context

      assert Queries.fetch_device_id_and_ownership_voucher(random_guid()) ==
               {:error, :not_found}
    end
  end
