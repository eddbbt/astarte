...
defmodule Astarte.FDO.OwnerOnboarding do
  ...
  def hello_device(cbor_hello_device) do
    with {:ok, hello_device} <- HelloDevice.decode(cbor_hello_device),
         ...
         {:ok, {device_id, ownership_voucher, realm_name}} <-
           OwnershipVoucher.fetch_with_device_id(guid),
  ...
  def ov_next_entry(cbor_body, guid) do
    ...
    with {:ok, %GetOVNextEntry{entry_num: entry_num}} <- GetOVNextEntry.decode(cbor_body),
         {:ok, {_realm, ownership_voucher}} <- OwnershipVoucher.fetch(guid) do
  ...
  def prove_device(body, session) do
    ...
    with {:ok, {_realm, ownership_voucher}} <- OwnershipVoucher.fetch(guid),
  ...
  defp add_output_voucher(ov_entry, to2_session) do
    with {:ok, {_realm, ownership_voucher}} <- OwnershipVoucher.fetch(to2_session.guid),
